function dKudC = op_D_DC_gradu_nu_gradv_times_u_mp_eval(spu, spuEval, spv, spvEval, spg, spgEval, msh, mshEval, u, mat, patch_list)

    indices = cell(msh.npatch, 1);
    values = cell(msh.npatch, 1);
    
    for iPatch = patch_list
        [ind, vals] = op_D_DC_gradu_nu_gradv_times_u(spuEval{iPatch}, spvEval{iPatch}, spgEval{iPatch}, mshEval{iPatch}, u(spu.gnum{iPatch}), mat);
