...
opts.Phase0 = OptimizationHistory(end).Parameters(17).Value;
opts.POLES = 4;
...
optiPoints =    {348, 344, 366, 367, 368, 369, 370, 371, 372, 373, 374, 375, 376, 354, 394, 395, 396, 397, 392, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 331, 334};
optiPointsSym = {227, 223, 245, 246, 247, 248, 249, 250, 251, 252, 253, 254, 255, 233, 273, 274, 275, 276, 271, 284, 285, 286, 287, 288, 289, 290, 291, 292, 293, 294, 282, 333};
...
Stator.addOptimizationParameter("Phase0", opts.Phase0, -deg2rad(20), deg2rad(20));

Rotor.GeometryFunction = @(p) loadRotor(p);
...
Motor.addOptimizationConstraint("TorqueMEAN", ">", 0.5);
...
function [srf, patches, opts] = loadRotor(opts)
    geo = mp_geo_read_nurbs('../opt_param/plots/final/RotorOpt50.txt');
    srf = vertcat(geo.nurbs); 
    patches = [];
    opts = [];
end