...
folder = '../opt_param_shape/plots/final';
srfI = [0, 1, 2, 3, 5, 10, 50, 100, 200, 284];
for isrf = 1:numel(srfI)
    srfNr = srfI(isrf);
    srfName = ['/RotorOpt' num2str(srfNr) '.txt'];

    OptimizationHistory = load([folder '/OptimizationHistory']).OptimizationHistory;
    opts.MA = OptimizationHistory(srfNr+2).Parameters(8).Value;
    opts.Phase0 = OptimizationHistory(srfNr+2).Parameters(46).Value;
    ...
    exportgraphics(ax,['plots/JMAGparamshape' num2str(srfNr) '.pdf']) 
    exportgraphics(ax,['plots/JMAGparamshape' num2str(srfNr) '.png'], "Resolution", 300); 
